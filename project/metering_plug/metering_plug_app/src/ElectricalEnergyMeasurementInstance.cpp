...
bool sMeteringActive           = false;

} // namespace

void ElectricalEnergyMeasurementInstance::SetMeteringActive(bool active)
{
    sMeteringActive = active;
    if (!active)
    {
...
CHIP_ERROR ElectricalEnergyMeasurementInstance::Init()
{
    ...
    SetMeteringActive(PlugMgr().IsPlugOn());
...
{
    if (!sMeteringActive || !PlugMgr().IsPlugOn())
    {
        sSecondsSinceUpdate = 0;
        return;
    }
