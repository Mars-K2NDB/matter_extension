# Matter Thread Battery Switch (BRD2703A)

4 个 Battery Switch 端点（endpoint 1-4），Matter Switch 集群事件逻辑参考 `connectedhomeip/examples/generic-switch-app`。

**电池供电 ICD 模式**：Thread MTD/SED + ICD Management / Time Synchronization，空闲时进入低功耗，按键唤醒发送 Switch 事件。
...
```
project/battery_switch_remote/artifact/matter_thread_soc_battery_switch_remote_app_series_2_internal_freertos_full.s37
```
...
4. 若曾烧录过常供电 FTD 固件，请 **长按 BTN0 恢复出厂** 后再测

## 编译

```bash
./build.sh battery_switch_remote
```

工程目录内也可直接调用（含 SLC generate 后的 codegen 补丁）：

```bash
python3 project/battery_switch_remote/build.py -b brd2703a
python3 project/battery_switch_remote/build.py -s   # 仅重编译，跳过 generate/补丁
```
