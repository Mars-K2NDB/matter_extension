...
constexpr uint32_t kPollIntervalActiveMs   = 5;
#if CHIP_CONFIG_ENABLE_ICD_SERVER
constexpr uint32_t kPollIntervalIcdIdleMs  = 30;
constexpr uint32_t kFastPollHoldMs         = 600;
#endif
...
#if CHIP_CONFIG_ENABLE_ICD_SERVER
TickType_t sFastPollUntilTick = 0;
#endif
...
#if CHIP_CONFIG_ENABLE_ICD_SERVER
void MarkFastButtonPoll()
{
    sFastPollUntilTick = xTaskGetTickCount() + pdMS_TO_TICKS(kFastPollHoldMs);
}

TickType_t GetButtonPollWaitTicks()
{
    if (sKeyDownMask != 0 || xTaskGetTickCount() < sFastPollUntilTick)
    {
        return pdMS_TO_TICKS(kPollIntervalActiveMs);
    }
    return pdMS_TO_TICKS(kPollIntervalIcdIdleMs);
}
#else
TickType_t GetButtonPollWaitTicks()
{
...
bool IsSwitchPinPhysicallyPressed(const sl_button_t * handle)
{
    if (handle == nullptr || handle->context == nullptr)
    {
        return false;
...
bool AnySwitchPinPressed()
{
    for (uint8_t i = APP_SWITCH_BUTTON_0; i <= APP_SWITCH_BUTTON_3; i++)
    {
        const sl_button_t * handle = SL_SIMPLE_BUTTON_INSTANCE(i);
        if (IsSwitchPinPhysicallyPressed(handle))
        {
            return true;
        }
    }
...
void DispatchSwitchButton(uint8_t button, uint8_t btnAction)
{
    ...
    const EndpointId endpointId = EndpointForButton(button);
    if (IsButtonPressed(btnAction))
    {
...
void PollAllButtons()
{
    ...
    for (uint8_t i = 0; i < SL_SIMPLE_BUTTON_COUNT; i++)
    {
        ...
        if (i == APP_RESET_BUTTON)
        {
            AppTask::ButtonEventHandler(i, state);
            continue;
        }

        if (i >= APP_SWITCH_BUTTON_0 && i <= APP_SWITCH_BUTTON_3)
        {
            DispatchSwitchButton(i, state);
        }
    }
}
...
void AppTask::AppTaskMain(void * pvParameter)
{
    ...
    while (true)
    {
        ...
        osStatus_t eventReceived = osMessageQueueGet(sAppEventQueue, &event, NULL, GetButtonPollWaitTicks());
...
void AppTask::ButtonEventHandler(uint8_t button, uint8_t btnAction)
{
    ...
    DispatchSwitchButton(button, btnAction);
}
