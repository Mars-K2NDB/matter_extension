...
**常供电 FTD 模式**：ZAP 已移除 ICD Management / Time Synchronization（上游 generic-switch-app 面向低功耗 ICD，与 FTD Router 冲突会导致启动失败 `67108891`）。
...
4. 若曾烧录过含 ICD 集群的旧固件，请 **长按 BTN0 恢复出厂** 后再测
...
```bash
python3 project/scripts/setup_generic_switch_project.py
```
