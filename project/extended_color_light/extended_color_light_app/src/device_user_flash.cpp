...
void ApplyCachedLightStateToMatter(EndpointId endpoint)
{
    ...
    uint16_t ct_mireds = s.ct_mireds;
    if (ct_mireds < light_output::kCtMinMireds || ct_mireds > light_output::kCtMaxMireds)
    {
        ct_mireds = light_output::kDefaultCtMireds;
    }
    ColorControl::Attributes::ColorTemperatureMireds::Set(endpoint, ct_mireds);
    ...
    ChipLogProgress(DeviceLayer, "Light state: applied to Matter (ct=%u level=%u on=0)", ct_mireds, level_to_apply);
}
...
void SaveLightStateFromMatter(EndpointId endpoint)
{
    ...
    if (ColorControl::Attributes::ColorMode::Get(endpoint, &mode) == Status::Success)
    {
        ...
        case ColorControl::ColorModeEnum::kCurrentHueAndCurrentSaturation:
            s.color_mode = LightColorMode::kHsv;
            break;
        case ColorControl::ColorModeEnum::kCurrentXAndCurrentY:
            s.color_mode = LightColorMode::kXy;
            break;
        default:
            s.color_mode = LightColorMode::kCt;
            break;
        }
    }
...
void UpdateLightStateFromAttributeChange(EndpointId endpoint, ClusterId cluster_id, AttributeId attribute_id)
{
    ...
    const bool relevant = (cluster_id == LevelControl::Id && attribute_id == LevelControl::Attributes::CurrentLevel::Id) ||
