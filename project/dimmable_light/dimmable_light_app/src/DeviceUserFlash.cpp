...
void ApplyCachedLightStateToMatter(EndpointId endpoint)
{
    ...
    ChipLogProgress(DeviceLayer, "Light state: applied to Matter (level=%u on=0)", levelToApply);
}
...
void UpdateLightStateFromAttributeChange(EndpointId endpoint, ClusterId clusterId, AttributeId attributeId)
{
    ...
    const bool relevant =
        (clusterId == LevelControl::Id && attributeId == LevelControl::Attributes::CurrentLevel::Id);
